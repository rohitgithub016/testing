...
const SelectChatType = () => {
  ...
  const previousRoute = searchParams.get("previous-route");
  ...
  const [groupsCount] = useState(groupsData?.length || 0);
  ...
  useEffect(() => {
    window.addEventListener("focus", clearIntervalFunction);
    return () => {
      window.removeEventListener("focus", clearIntervalFunction);
      clearInterval(timeoutInterval);
    };
  }, []);

  const createChainLinkInvite = (group: Groups) => {
    if (selectedChainLinkType === "GateKeeper") {
      navigate(
        `/create-gate-keeper-link?groupId=${group?._id}&groupName=${group?.groupName}&chatType=tokengated`
      );
      return;
    }
    if (selectedChainLinkType === "AccessPass") {
      navigate(
        `${ACCESS_PASS}?chat-type=AccessPass&group-name=${group?.groupName}&group-id=${group?._id}`
      );
      return;
    }
    navigate(
      `/add-invite-details?groupId=${group?._id}&groupName=${group?.groupName}&chatType=chainsync`
    );
  };

  useEffect(() => {
    if (groupsData?.length > groupsCount) {
      const group = groupsData[groupsData?.length - 1];
      createChainLinkInvite(group);
    }
  }, [groupsData]);
  ...
  const [selectedChainLinkType, setSelectedType] = useState("GateKeeper");
  ...
  let progressPercentage = [100, 100, 50];

  if (onboardingCompleted === "true") {
    progressPercentage = [0, 0, 0];
  }
  ...
  return (
    <Page loading={isFetching}>
      ...
      >
            ...
            <Flex vertical gap={24}>
              <Flex vertical>
                ...
                <Flex gap={16} vertical>
                  {chainLinkType?.map(({ type, description }) => (
                    <Card
        ...
        <Flex style={{ padding: "0px 16px 30px 16px" }}>
          <Button
